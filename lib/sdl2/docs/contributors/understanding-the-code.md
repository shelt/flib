...
- Declaring type aliases for enums and other integer-like types, for
  ...
  descriptive). E.g. `SDL_EventType` is an alias for `int`.
...
informative message. The `noop-guard` is a guard procedure that does
nothing, for fields where you don't need/want validation.
...
`pred:`, `print:`, `type:` `getter:`, `setter:`, and `guard:`.
